...
export class GalaxyEnvironment {
  ...
  makeStars(points) {
    for(const [layer,count,span,color,size] of [[this.far,3000,150,'#b7d2e8',.24],[this.middle,1800,110,'#8fbfcf',.32]]){
      const p=[],s=[],f=[];
      for(let i=0;i<count;i++){p.push((this.random()-.5)*span,(this.random()-.5)*span*.72,(this.random()-.5)*18);s.push(.07+this.random()*size);f.push(this.random()*6.28);}
      const stars=points(p,s,f,color,.96);layer.add(stars);this.animatedPoints.push(stars);
    }
    ...
    for(let i=0;i<4800;i++){
      ...
      s.push(.085+Math.pow(this.random(),3)*.44);f.push(this.random()*6.28);
    }
    this.dust=points(p,s,f,'#a3bdff',.75);this.middle.add(this.dust);this.animatedPoints.push(this.dust);
    const brightPositions=[],brightSizes=[],brightPhases=[];
    for(let i=0;i<120;i++){
      const x=(this.random()-.5)*100;
      const y=i<75?x*.43+Math.sin(x*.075)*4+(this.random()-.5)*15-4:(this.random()-.5)*65;
      brightPositions.push(x,y,(this.random()-.5)*14);brightSizes.push(.55+Math.pow(this.random(),2)*1.6);brightPhases.push(this.random()*6.28);
    }
  ...
  nebulaTexture() {
    ...
    for(let i=0;i<140;i++){
      ...
      cloud(x,y,70+this.random()*130,color,.035+this.random()*.06,.5+this.random()*.4);
    }
  ...
  update(dt,time,orientation,detail,motion) {
    ...
    this.nebula.material.opacity=.95-this.detailMix*.12;
    this.haze.material.opacity=.25-this.detailMix*.035;
